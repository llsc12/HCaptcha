...
extension String {
  fileprivate func jsSanitize() -> String {
    return self.replacingOccurrences(of: ".", with: "_")
  }

  fileprivate var isSystemFramework: Bool {
...
private func bundleShortVersion() -> String {
  ...
  let sdkBundleShortVer =
    sdkBundle.object(forInfoDictionaryKey: "CFBundleShortVersionString")
    as? String
...
class HCaptchaDebugInfo {
  ...
  private class func buildDebugInfo() -> [String] {
    ...
    if loadedCount > 0 {
      ...
      _ = objc_getClassList(
        AutoreleasingUnsafeMutablePointer(classes), Int32(loadedCount))

      for idx in 0..<loadedCount {
        if let `class` = classes[idx] {
          var info = Dl_info()
          if dladdr(unsafeBitCast(`class`, to: UnsafeRawPointer.self), &info)
            != 0,
    ...
    #elseif os(macOS)
      let iver = ProcessInfo.processInfo.operatingSystemVersionString
        .jsSanitize()
    #endif
    return [
